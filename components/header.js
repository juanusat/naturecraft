...
export const createHeader = () => {
    let dataAccount = localStorage.getItem('dataSite_account') || 0
    let site_mark = typeof site_location == 'string' ? site_location : ''
    if ((['register', 'login']).includes(site_mark)) {
        const header = createElement('header', {
            className: 'login-page maximize',
            innerHTML: /*html*/ `
