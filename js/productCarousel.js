...
const generateStars = (rating) => {
    ...
    for (let i = 1; i <= 5; i++) {
        starsHTML += i <= rating
            ? '<i class="fa-solid fa-star" style="color: #ffcc00;"></i>' // Estrella llena
            : '<i class="fa-regular fa-star" style="color: #ccc;"></i>'; // Estrella vacía
    }
    return starsHTML;
};
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    products.forEach((product) => {
        ...
        productCard.classList.add('card-product-item-reg');
        productCard.innerHTML = `
            ...
            <div class="details">
                <div class="rating">Estrellas ${generateStars(product.rating)}</div>
    ...
    let currentIndex = 0;
    const productWidth = document.querySelector('.card-product-item-reg').offsetWidth + 20; 
